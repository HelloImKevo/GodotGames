extends Node
## PlayerManager singleton, used in conjunction with the MultiplayerInput
## addon, by matjlars:
## https://github.com/matjlars/godot-multiplayer-input

# Player is 0-3
# Device is -1 for keyboard/mouse, 0+ for joypads
# these concepts seem similar but it is useful to separate them so for example,
# device 6 could control player 1.
...
func join(device: int, player_name: String):
	var player: int = next_player()
	if player >= 0:
...
# Get player data. Null means it doesn't exist.
...
# Call this from a process loop in the main menu or anywhere they can
# join this is an example of how to look for an action on all devices.
